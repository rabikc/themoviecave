...
const UserContentCard = ({content, getRated}) => {

  const { contextData } = useContext(AuthContext);

  const deleteRated = async () => {
      
    const response = fetch(`http://127.0.0.1:8000/api/rated/${content.id}`, {
      method: "DELETE",
      headers: {
  ...
  return (
    <div className="single-content">
      ...
      <div className="user-content-card-detail">
        ...
        <button className="content-remove-btn" onClick={deleteRated}>Remove</button>
      </div>
    </div>
  );
};
