...
const Tv = () => {
  ...
  const fetchMovies = async () => {
    ...
    const { data } = await axios.get(
      `${rootUrl}/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&language=en-US
  ...
  const search = async (e) => {
    try {
      const { data } = await axios.get(
        `${rootUrl}/search/tv?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&query=${searchText}&page=${page}&include_adult=false`
      )
      setContent(data.results);
      setNumOfPages(data.total_pages)
    }
  ...
  const top = async (e) => {
    try {
      const { data } = await axios.get(
        `${rootUrl}/tv/top_rated?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&${page}`
      )
      setContent(data.results);
      setNumOfPages(data.total_pages);
      console.log(data)
  ...
  const popular = async (e) => {
    try {
      const { data } = await axios.get(
        `${rootUrl}/tv/popular?api_key=${process.env.REACT_APP_API_KEY}&language=en-US${page}`
      )
      setContent(data.results);
      setNumOfPages(data.total_pages);
      console.log(data)
  ...
  const onAir = async (e) => {
    try {
      const { data } = await axios.get(
        `${rootUrl}/tv/on_the_air?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&${page}`
      )
      setContent(data.results);
      setNumOfPages(data.total_pages);
      console.log(data)
